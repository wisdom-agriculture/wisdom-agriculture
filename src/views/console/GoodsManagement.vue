<template>
    <div class="traceContainer">
        <div class="tableControllBar">
            <div>
                <a-button type="primary" @click="router.push('GoodsEdit')">
                    <plus-outlined /> 添加作物
                </a-button>
            </div>
            <div>
        ...
        <a-table :columns="columns" :data-source="data" :pagination="{ pageSize: 20 }" :row-selection="rowSelection">
            <template #bodyCell="{ column, record }">
                <template v-if="column.key==='producesStatus'">
                    <a-switch v-model:checked="record.producesStatus" @change="switchStatus(record)" />
                </template>
                <template v-if="column.key === 'action'">
                    <a-button type="text" style="color:#4095E5" @click="router.push('GoodsEdit')">修改</a-button>
                    <a-button v-if="column.key==='asdf'" type="text" style="color:rgba(189,49,36,1)" @click="deleteItem(record)">删除</a-button>
                </template>
            </template>
        </a-table>
    </div>
...
import apis, { Goods } from '@/tools/apis';
...
const columns = [
    ...
    {
        dataIndex: 'producesName',
        title: '商品名称',
        key: 'producesName',
        align: 'center'
    },
    {
        dataIndex: 'producesPrice',
        title: '商品价格（元）',
        key: 'producesPrice',
        align: 'center'
    },
    {
        dataIndex: 'producesStatus',
        title: '是否上架',
        key: 'producesStatus',
        align: 'center'
    },
...
const con = console
const data = ref<Goods.goodsItem[]>([])
const getData = () => {
    apis.goods.getGoodsList().then(res => {
        if (res.status == 'success') {
            data.value = res.data.data.producesManagementResults
        }
    })
}
getData()
const switchStatus=(record:Goods.goodsItem)=>{
    apis.goods.switchStatus({
        id:String(record.id),
        producesStatus:record.producesStatus?"1":"0"
    })
}
const deleteItem = (record: Goods.goodsItem) => {
    apis.goods.deleteProduct({items:[record.id]}).then(res=>{
        if(res.status=='false'){
...
const rowSelection = ref({
    checkStrictly: false,
    onChange: (selectedRowKeys: (string | number)[], selectedRows: Goods.goodsItem[]) => {
        console.log(`selectedRowKeys: ${selectedRowKeys}`, 'selectedRows: ', selectedRows);
    },
    onSelect: (record: Goods.goodsItem, selected: boolean, selectedRows: Goods.goodsItem[]) => {
        console.log(record, selected, selectedRows);
    },
    onSelectAll: (selected: boolean, selectedRows: Goods.goodsItem[], changeRows: Goods.goodsItem[]) => {
