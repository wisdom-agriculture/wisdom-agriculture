<template>
    <div class="traceContainer">
        ...
        <a-table :columns="columns" :data-source="data" :pageSize="18"  :row-selection="rowSelection">
            <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'action'">
                    <template v-if="record.status===0">
                        <a-button type="text" style="color:#4095E5" @click="startTransport(record.id)">开始运输</a-button>
                    </template>
                    <template v-else-if="record.status===1">
                        <a-button type="text" style="color:#4095E5" @click="endTransport(record.id)">
                            配送完毕</a-button>
                    </template>
                </template>
                <template v-if="column.key === 'status'">
                    <div v-if="record.status === 0">
                        <a-tag color="blue">未运输</a-tag>
                    </div>
                    <div v-if="record.status === 1">
                        <a-tag color="blue">运输中</a-tag>
                    </div>
                    <div v-if="record.status === 2">
                        <a-tag color="red">运输完成</a-tag>
                    </div>
                </template>
            </template>
        </a-table>
...
const data = ref<Trace.Driver.transportItem[]>([])
const getData = () => apis.trace.driver.getTransportList().then(res => {
...
const rowSelection = ref({
    checkStrictly: false,
    onChange: (selectedRowKeys: (string | number)[], selectedRows: Trace.Driver.transportItem[]) => {
        console.log(`selectedRowKeys: ${selectedRowKeys}`, 'selectedRows: ', selectedRows);
    },
    onSelect: (record: Trace.Driver.transportItem, selected: boolean, selectedRows: Trace.Driver.transportItem[]) => {
        console.log(record, selected, selectedRows);
    },
    onSelectAll: (selected: boolean, selectedRows: Trace.Driver.transportItem[], changeRows: Trace.Driver.transportItem[]) => {
        console.log(selected, selectedRows, changeRows);
    },
});

const startTransport = (id: number) => Modal.confirm({
    content: '是否确认开始运输货物',
    onOk() {
        apis.trace.driver.startTransport({ id }).then(res => {
            if (res.status == 'success') getData()
            else if (res.status == 'false') message.error(res.message)
        })
    },
    onCancel() {
        console.log('Cancel');
...
const endTransport = (id: number) => Modal.confirm({
    content: '是否确认配送完毕',
    onOk() {
        apis.trace.driver.finishTransport({ id }).then(res => {
            if (res.status == 'success') getData()
            else if (res.status == 'false') message.error(res.message)
        })
    },
