<template>
    <div class="traceContainer">
        <trace-code :visible="traceCodeVisible" :closer="traceCodeCloser"></trace-code>
        <trace-i-d :crops-id="cropsIdSelected" :visible="traceIDVisible" :closer="traceIDCloser"></trace-i-d>
        ...
        <a-table :loading="loading" :columns="columns" :data-source="data" :pagination="{ pageSize: 20 }"
            ...
            <template #bodyCell="{ column, record }">
                ...
                <template v-if="column.key === 'action'">
                    <template v-if="record.retailerReceiveStatus==0">
                        <a-button type="text" style="color:#4095E5" @click="confirmReceive(record.id)">确认收货</a-button>
                    </template>
                    <template v-else>
                        <a-button type="text" style="color:#4095E5" @click="traceIDOpen(record.cropsId)">
                            获取产品溯源ID</a-button>
                    </template>
                </template>
                <template v-if="column.key === 'retailerReceiveStatus'">
                    <div v-if="record.retailerReceiveStatus == true">
                        <a-tag color="blue">已收货</a-tag>
                    </div>
                    <div v-if="record.retailerReceiveStatus == false">
                        <a-tag color="red">未收货</a-tag>
                    </div>
                </template>
            </template>
        </a-table>
...
import TraceCode from '@/components/traceBack/TraceCode.vue';
import TraceID from '@/components/traceBack/TraceID.vue';
...
const columns = [
    ...
    {
        dataIndex: 'retailerReceiveStatus',
        title: '签收状态',
        key: 'retailerReceiveStatus',
        align: 'center'
    }, {
        key: 'action',
        title: '操作',
...
// const data = ref<Trace.Retailer.plantItem[]>([{
//     key: 0,
//     serial: 'asdfasdfasdfa',
//     name: '张三',
...
const data = ref<Trace.Retailer.plantItem[]>([])
const getData = () => {
    apis.trace.retailer.getPlantList().then(res => {
...
const rowSelection = ref({
    checkStrictly: false,
    onChange: (selectedRowKeys: (string | number)[], selectedRows: Trace.Retailer.plantItem[]) => {
        console.log(`selectedRowKeys: ${selectedRowKeys}`, 'selectedRows: ', selectedRows);
    },
    onSelect: (record: Trace.Retailer.plantItem, selected: boolean, selectedRows: Trace.Retailer.plantItem[]) => {
        console.log(record, selected, selectedRows);
    },
    onSelectAll: (selected: boolean, selectedRows: Trace.Retailer.plantItem[], changeRows: Trace.Retailer.plantItem[]) => {
        console.log(selected, selectedRows, changeRows);
    },
});

const confirmReceive = (id: number) => Modal.confirm({
    content: '是否确认收货',
    onOk() {
        apis.trace.retailer.receive({ id }).then(res => {
            if (res.status == 'success') {
...
const traceCodeVisible = ref(false);
const traceCodeCloser = () => {
    traceCodeVisible.value = false;
}
const traceCodeOpen = () => {
    traceCodeVisible.value = true
}

const cropsIdSelected=ref<bigint>(0n)
const traceIDVisible = ref(false);
const traceIDCloser = () => {
    traceIDVisible.value = false;
}
const traceIDOpen = (cropsId:bigint) => {
    cropsIdSelected.value=cropsId
    traceIDVisible.value = true
}
